...
MSE = np.mean((T_a - Xe_all[1, :]) ** 2)
print('Mean Squared Error for azimuth :', MSE)
...
plt.subplot(131)
plt.scatter(time[::8], T_r[::8], color='g', label='True Azimuth', linewidth=1)
...
plt.subplot(132)
plt.scatter(time[::8], m_r[::8], color='r', label='Measured Azimuth', linewidth=1)
plt.xlabel('Time (Sec) - starting from 9hr 58min')
plt.ylabel('Azimuth')
...
plt.subplot(133)
plt.scatter(time[::8], Xe_all[0, ::8], color='b', label='Estimated Azimuth', linewidth=1)
plt.xlabel('Time (Sec) - starting from 9hr 58min')
plt.ylabel('Azimuth')
